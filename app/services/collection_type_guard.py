...
def ensure_vector_collection(collection) -> None:
    ...
    if ctype not in (COLLECTION_TYPE_VECTOR, COLLECTION_TYPE_COMPOSITE):
        raise HTTPException(
...
def ensure_vector_data_collection(collection) -> None:
    """Vector collection that stores features (not composite)."""
    ctype = getattr(collection, "collection_type", COLLECTION_TYPE_VECTOR)
    if ctype != COLLECTION_TYPE_VECTOR:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="This endpoint is only available for vector collections with stored features.",
        )


def ensure_composite_collection(collection) -> None:
    if getattr(collection, "collection_type", "") != COLLECTION_TYPE_COMPOSITE:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="This endpoint is only available for composite collections.",
        )
